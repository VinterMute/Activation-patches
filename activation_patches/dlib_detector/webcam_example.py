...
from dlib_facedetector_pytorch.dlib_torch_converter import get_model
...
def detect_multi_scale(model, img, scales, recepive_field):
	...
	for count,  scale in enumerate(scales):
			...
			output1 = detect_img(model, np.float32(img))
			output2 = sk_resize(output1, output_shape=img.shape[:2], preserve_range=True)
			output = np.float32(output2)
			# cv2.imwrite(f"test/{count}.jpg", output)
